...
- **연동 현황 & 안내:**
  ...
  - **프론트엔드 연동 가이드 & 중요 설계:**
    ...
    - **⭐ 완전한 관심사 분리 및 지능형 상태 기억 시스템:**
      *   꿈 해몽 요청 시 바디에는 고민/기분을 전혀 전송하지 않고 오직 **꿈 내용(`content`)**만 전송하므로 화면 구성이 극도로 미니멀하고 직관적입니다.
      *   사용자가 개인 프로필 화면이나 메인페이지의 간단 상태 팝업 모달을 통해 **`PUT /api/user/status`**로 자신의 상황을 저장해 두면, 백엔드가 해몽 시 자동으로 이 값을 데이터베이스에서 찾아 AI의 심리 분석 틀에 주입해 줍니다.
      *   이로써 유저는 단지 꿈만 가볍게 적었을 뿐인데도, 역도사 AI가 **유저의 현실 고민과 기분 상태를 은유적으로 꿰뚫어 보며 멘토링해 주는 듯한 소름 돋는 사용자 경험**을 선사합니다!
