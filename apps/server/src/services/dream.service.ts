...
export class DreamService {
  static async interpretDream(
    ...
    const personaRules = `
      ...
      - Rule 2 (Reversed Strictness): 타로 카드가 '역방향'인 경우 사용자가 애써 외면하고 있는 '불편한 진실'이나 '그릇된 습관'을 정면으로 냉정하게 지적(팩트 폭행)하시오.
    ...
    const prompt = `
      ...
      [작업 지시]
      1. 주입된 3장의 타로 카드 정보와 사용자의 꿈 서사를 절묘하게 융합하여 해석하되, 정/역방향 규칙과 페르소나를 철저히 고수하시오.
      ...
      [출력 형식 (정밀 JSON)]
      {
        ...
        "tarotAnalysis": [
          { "card": "카드이름", "status": "정방향/역방향", "meaning": "카드 자체의 날카로운 해석", "advice": "이 카드가 주는 현실적 뼈 때리는 조언" }
        ],
