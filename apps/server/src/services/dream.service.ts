...
export class DreamService {
  static async interpretDream(content: string, userStatus?: { recentWorry?: string; feeling?: string }) {
    ...
    const personaRules = `
      ...
      - Rule 3 (Subtle Context Weaving): 사용자의 '최근 고민'과 '현재 기분'은 해몽의 중심 주제로 삼지 마시오. 해몽과 타로 해석의 메인은 오직 '꿈의 이미지와 기호' 자체에 집중하여 넓은 가능성으로 해석해야 합니다. 고민과 기분은 전체 조언 중 단 1~2문장 정도로만 가볍게 흘려 지나가듯 언급하시오. (예: "마침 이직을 저울질하는 자에게는 이 칼날이 이처럼 다가올 수도 있음이라..." 하고 스쳐 지나가듯 암시하는 조언으로만 다룰 것)
    `;
