...
const blocks = [
  ...
  },
  {
    ...
    points: [
      ...
      '조선 시대 사람들은 좋은 꿈을 사고팔기도 했으며,',
      '최근에는 이를 증명하는 문서도 발견되었습니다. (한국국학진흥원)',
    ],
    image: Story2Image,
  },
  {
    ...
    <section id='story' className='bg-yangrok-900 px-6 py-24'>
      <div className='mx-auto flex max-w-5xl flex-col gap-16 text-center'>
        {blocks.map((b) => (
          ...
          >
            {/* 이미지 */}
            <img src={b.image} alt={b.heading} className='w-[400px] h-auto' />

            {/* 텍스트 */}
            <div className='flex flex-col items-start text-left'>
              <h2 className='text-2xl font-bold leading-snug text-guncheong-500'>
                {b.heading}
              </h2>
              <ul className='mt-6'>
                {b.points.map((p) => (
                  <li key={p} className='text-md leading-relaxed text-ink-body'>
                    {p}
                  </li>
                ))}
              </ul>
